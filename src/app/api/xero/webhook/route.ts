...
async function processInvoiceEvent(
  ...
  // Find the matching local invoice first. Xero fires INVOICE webhooks for
  // EVERY invoice in the tenant — supplier bills, payroll, manual stuff, the
  // lot. Most of those have no business hitting our app. If we fetch from
  // Xero before checking the local DB, we burn one Xero API call per noisy
  ...
  // No local row. The only reason we'd still care is if the invoice was
  // auto-generated by Xero from one of our RepeatingInvoice templates.
  // Xero only fires CREATE for these — never UPDATE without a prior CREATE
  // that we'd already have ingested. So:
  //   - eventType === UPDATE & no local row → not ours, drop without fetching.
  //   - eventType === CREATE & no local row → only fetch if we have any
  //     active recurring plans that could plausibly own it.
  if (!invoice) {
    ...
    // We have at least one active recurring plan — fetch to check if this
    // CREATE is one of its children.
  ...
  // We have a local row — this is one of ours, fetch to refresh.
  const { client, conn } = await getAuthedClient(supabase);
  const latest = await fetchXeroInvoice(client, conn.tenant_id, xeroInvoiceId);
