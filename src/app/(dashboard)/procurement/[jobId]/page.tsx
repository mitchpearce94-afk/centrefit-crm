...
export default async function ProcurementJobPage({
  ...
  const [jobResult, procurementResult, suppliersResult, quoteResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "*, received_by_staff:staff!job_procurement_items_received_by_fkey(display_name), product:quote_products(category, cost_price), line:quote_line_items(cost_price)",
      )
      .eq("job_id", jobId)
