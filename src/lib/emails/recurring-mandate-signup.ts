  ...
  const intro = `
    ...
    <p style="font-size:14px;line-height:1.6;color:#111827;margin:0 0 18px">
      ${links.length === 1
        ...
        : `You're being onboarded for direct debit at <strong>${links.length} sites</strong>. Each site needs its own authorisation so the right facility's invoices flow against the right account. Please click each button below and complete the form — same bank details, just enter them once per site.`
      }
    </p>
    <div style="border-left:3px solid #2563eb;background:#eff6ff;padding:12px 14px;margin:0 0 18px;border-radius:0 6px 6px 0">
      <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#1e3a8a">A note on the email shown on the form</p>
      <p style="margin:0;font-size:12px;line-height:1.55;color:#1e3a8a">
        Your email will appear ${links.length === 1 ? "as" : "with a different"} <span style="font-family:Consolas,Menlo,monospace">you+sitename@your-domain.com</span>${links.length > 1 ? " for each site" : ""}.
        That's intentional — it's locked on the form and is how we route ${links.length === 1 ? "this site's" : "each site's"}
        invoices to the right facility on our end. Messages to that variant still land in your normal inbox.
      </p>
    </div>`;
