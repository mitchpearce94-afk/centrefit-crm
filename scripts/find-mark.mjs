// Find Mark Pearce's Xero contact + email, if any.
...
const w = encodeURIComponent('Name.Contains("Pearce")');
const r = await (await fetch(`https://api.xero.com/api.xro/2.0/Contacts?where=${w}`, {
  headers: { Authorization: `Bearer ${tok}`, "Xero-tenant-id": conn.tenant_id, Accept: "application/json" },
})).json();
for (const c of r.Contacts ?? []) console.log(`${c.ContactID} | ${c.Name} | ${c.EmailAddress ?? "(no email)"} | ${c.ContactStatus}`);
if (!(r.Contacts ?? []).length) console.log("no Pearce contacts in Xero");
