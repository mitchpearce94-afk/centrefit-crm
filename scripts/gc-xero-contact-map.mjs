...
// for the agent). Writes scripts/gc-xero-contact-map.csv + .json, changes nothing.
...
console.log("\nNEEDS A DECISION:");
for (const r of rows.filter((r) => !r.status.startsWith("OK"))) console.log(`  [${r.status}] ${r.gc_name.slice(0, 34).padEnd(34)} subs ${r.gc_subs}  plan ${r.crm_active_plans}/${r.crm_plans}  site "${r.crm_site.slice(0, 26)}"  stored "${r.stored_xero_contact.slice(0, 30)}"  → ${r.proposed_xero_contact.slice(0, 34) || r.candidates.slice(0, 70)}`);
const csvEsc = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
const cols = Object.keys(rows[0] ?? {});
writeFileSync(new URL("./gc-xero-contact-map.csv", import.meta.url), [cols.join(","), ...rows.map((r) => cols.map((c) => csvEsc(r[c])).join(","))].join("\n"));
writeFileSync(new URL("./gc-xero-contact-map.json", import.meta.url), JSON.stringify({ generatedAt: new Date().toISOString(), counts, rows }, null, 2));
console.log(`\nwrote scripts/gc-xero-contact-map.csv (${rows.length} rows) + .json`);
