// 2026-07-07 — READ-ONLY: locate Estella's pre-consolidation RIs (any contact) for backfill line items.
...
for (const r of all.filter((r) => /estella|puntarungsy/i.test(r.Contact?.Name ?? ""))) {
  console.log(`${r.Status.padEnd(10)} $${String(r.Total).padEnd(8)} "${r.Reference ?? ""}"  next=${xd(r.Schedule?.NextScheduledDate)}  contact="${r.Contact.Name}" (${r.Contact.ContactID.slice(0, 8)})  ${r.RepeatingInvoiceID.slice(0, 8)}`);
  for (const l of r.LineItems ?? []) console.log(`     - ${l.Description?.slice(0, 60)}  $${l.LineAmount}  acct=${l.AccountCode} item=${l.ItemCode ?? "-"} tax=${l.TaxType}`);
}
